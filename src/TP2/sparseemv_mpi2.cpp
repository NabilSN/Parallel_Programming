...
int main(int argc, char** argv)
{
  ...
  {
      sendcounts[i] = local_nrows;
      displs[i] = i * local_nrows;
  }

  int local_start_row = displs[rank];
  int local_end_row = local_start_row + local_nrows;

  double *local_y = new double[local_nrows];
...

  {
    ...
    MPI_Gatherv(local_y, local_nrows, MPI_DOUBLE,
                y2.data(), sendcounts, displs, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  }
  if (rank == 0)
  {
